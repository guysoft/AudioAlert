...
class AudioNet_1D:
    def __init__(self, lenght, class_num=2, sess=tf.Session()):
        ...
        with tf.variable_scope("first"):
            net = AudioNet_1D.conv(tf.expand_dims(self.in_sound, 2), 7, 16, stride, "conv1_0",
                                   regularizer=self.regularizer)
        # net = AudioNet_1D.conv(tf.expand_dims(self.in_sound,2), 25, 16, 1, "conv1", regularizer=self.regularizer)
        # Resnet Blocks
        with tf.variable_scope("BlockA"):
            for n in range(5):
                net = AudioNet_1D.resblock(net, 9, 16, 1, "BlockA_" + str(n), regularizer=self.regularizer)
        with tf.variable_scope("second"):
            net = AudioNet_1D.conv(net, 9, 32, stride, "conv2", regularizer=self.regularizer)
        ...
        with tf.variable_scope("thrid"):
            net = AudioNet_1D.conv(net, 9, 64, stride, "conv3", regularizer=self.regularizer)
        ...
        #     net_splt =tf.reshape( AudioNet_1D.conv(net_splt_3, 9, 1, 1, "clean_out", regularizer=self.regularizer, is_relu=False), (-1, lenght))
